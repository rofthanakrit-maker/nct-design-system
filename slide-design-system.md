...
| 10 | Closing / Contact | gradient teal→navy 45° + แถบภาพขวา + foot scrim (web มีโหมดภาพเต็มหน้าเพิ่ม) | title, **ขั้นตอนถัดไป (label + list), วันที่ต้องการคำตอบ**, contact | หน้าสุดท้ายเท่านั้น |
...
- **01 / 10** ใช้ gradient คนละทิศ (navy→teal เปิด, teal→navy ปิด) ตั้งใจให้เด็คมี
  bookend รู้สึกจบ — อย่าสลับ ทั้งสองมี **Footer Scrim** เต็มความกว้างที่ก้นสไลด์:
  ปลายสว่างของ gradient คือ `TEAL_B` ซึ่งต่อให้ตัวหนังสือขาวทึบก็ยังได้แค่ 4.0:1
  แก้ที่พื้น ไม่ใช่ที่หมึก — ห้ามลบ scrim นี้แล้วปล่อยแถบ footer ไว้บน gradient เปล่า
- **10** คือ "หน้าขอ" ไม่ใช่หน้าขอบคุณ ต้องมีขั้นตอนถัดไปพร้อมผู้รับผิดชอบ และวันที่
  ต้องการคำตอบ คำว่า `ขอบคุณครับ` เป็นแค่หัวเรื่องเหนือมัน โลโก้ตัวเต็มถูกถอดออกจาก
  layout นี้ (ไม่มีที่ว่างเหลือ) เครื่องหมายมุมล่างเป็นตัวเซ็นแทน
...
- **09** ใส่ตารางผ่านปุ่ม Insert Table ในตัว placeholder จะได้ theme style อัตโนมัติ
  และเป็นหน้าราคาด้วย: ใส่แถวค่าบริการเป็นแถวสุดท้ายแบบตัวหนา แล้วทำเครื่องหมาย
  คอลัมน์ที่แนะนำ (หัวคอลัมน์เป็น `TEAL` + แถบ `TEAL` 0.05in เหนือหัวตาราง)
  สามคอลัมน์น้ำหนักเท่ากันกับประโยคสรุปข้างล่าง ยังไม่ใช่การแนะนำ
